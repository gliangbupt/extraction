...
def write_excel(list_data,list_ext):
    ...
    for i in range(len(list_data)):
        ...
        worksheet.write(row+1,col+1,item['Texture_acutance_clipped'])
        worksheet.write(row+2,col,str('Edge_acutance'))
        worksheet.write(row+2,col+1,item['Edge_acutance'])
        worksheet.write(row +3 , col, str('Visual_Noise'))
        worksheet.write(row + 3, col+1,item['Visual_Noise'] )
        row+=4
    # 写入最大值数据
    row_2=2
    col_2=2
    ...
    for i in range(len(list_ext)):
        ...
        worksheet.write(row_2 + 1,col_2,item['Texture_acutance_clipped'])
        worksheet.write(row_2 + 2, col_2, item['Edge_acutance'])
        worksheet.write(row_2 + 3, col_2 , item['Visual_Noise'])
        col_2+=1
    workbook.close()
